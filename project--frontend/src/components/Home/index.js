...
class Home extends Component {
    ...
    addingTaskFunction = async () => {
        ...
        if (task !== '') {
            const response = await fetch('https://to-do-full-stack-application-3.onrender.com/task', {
                method: 'POST',
    ...
    deleteTaskFunction = async (id) => {
        try {
            const response = await fetch(`https://to-do-full-stack-application-3.onrender.com/task/${id}?name=${encodeURIComponent(this.state.name)}`, {
                method: 'DELETE',
            });
    ...
    getData = async () => {
        try {
            const response = await fetch(`https://to-do-full-stack-application-3.onrender.com/userTasks?name=${encodeURIComponent(this.state.name)}`);
